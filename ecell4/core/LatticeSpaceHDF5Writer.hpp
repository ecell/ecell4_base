...
#undef INSERT_MEMBER
                    ...
                    H5::Group *group)
    {
        ...
        // else
        //     property.location =
        //     H5std_string(loc->species().serial().c_str());
        if (loc->is_vacant())
        ...
        // mtgroup->createAttribute("location", H5::StrType(0, H5T_VARIABLE),
        // H5::DataSpace(H5S_SCALAR)
        //     ).write(H5::StrType(0, H5T_VARIABLE), &property.location);
        mtgroup
            ->createAttribute("is_structure", H5::PredType::STD_I32LE,
                              H5::DataSpace(H5S_SCALAR))
    ...
    static void save_voxel_pool_recursively(
        const Species &location,
        std::multimap<Species, const VoxelPool *> &location_map, Tspace_ &space,
        H5::Group *root)
    {
    ...
    static void
    sort_by_location(std::multimap<std::string, Species> location_map,
                        ...
                        const std::string &impl = "")
{
    ...
    traits_type::save_voxel_pool_recursively(
        space.vacant()->species(), location_map, space, spgroup.get());
...
#define CREATE_ATTRIBUTE(attribute, type)                                      \
    root->createAttribute(#attribute, type, H5::DataSpace(H5S_SCALAR))         \
...
#undef OPEN_ATTRIBUTE
    ...
    //         H5::StrType(0, H5T_VARIABLE), impl);  //XXX:  '&' is not needed
    //         for HDF5std_string
    // if (root.attrExists("implementation"))
    //     root.openAttribute("implementation").read(
    ...
    try
    {
        root.openAttribute("implementation")
            .read(H5::StrType(H5::PredType::C_S1, 32), impl_C);
    }
    ...
    std::map<Species, std::pair<traits_type::h5_species_struct,
                                std::vector<std::pair<ParticleID, Integer>>>>
        tmp_map;
    ...
    for (hsize_t idx(0); idx < spgroup.getNumObjs(); ++idx)
    {
        ...
        H5Lget_name_by_idx(spgroup.getLocId(), ".", H5_INDEX_NAME, H5_ITER_INC,
                           idx, name_C, 32, H5P_DEFAULT);
        ...
        // reading.
        group.openAttribute("location")
            .read(H5::StrType(H5::PredType::C_S1, 32), property.location);
        group.openAttribute("is_structure")
            .read(H5::PredType::STD_I32LE, &property.is_structure);

        // std::cout << "load_property(" << name << "," << property.radius <<
        // "," << property.D << "," << property.location << ");" << std::endl;

        struct_map.insert(std::make_pair(species, property));
        location_map.insert(
            std::make_pair(std::string(property.location), species));
        ...
        for (unsigned int idx(0); idx < num_voxels; ++idx)
        {
            voxels.push_back(std::make_pair(
                ParticleID(std::make_pair(h5_voxel_array[idx].lot,
