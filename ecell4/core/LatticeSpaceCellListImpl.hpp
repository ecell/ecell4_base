...
class LatticeSpaceCellListImpl : public HCPLatticeSpace
{
public:
...
public:
    LatticeSpaceCellListImpl(const Real3 &edge_lengths,
        ...
        : base_type(edge_lengths, voxel_radius, is_periodic),
          is_periodic_(is_periodic), matrix_sizes_(matrix_sizes),
    ...
    inline matrix_type::size_type
    coordinate2index(const coordinate_type &coord) const
    {
    ...
    {
        return (g.col / cell_sizes_[0]) +
               matrix_sizes_[0] *
                   ((g.row / cell_sizes_[1]) +
                    matrix_sizes_[1] * (g.layer / cell_sizes_[2]));
    }

    cell_type::iterator find_from_cell(const coordinate_type &coord,
                                       cell_type &cell)
    {
        return std::find_if(
            cell.begin(), cell.end(),
    ...
    cell_type::const_iterator find_from_cell(const coordinate_type &coord,
                                 ...
                                 const Integer &nrand) const
    {
        ...
        return (!is_periodic_ || is_inside(dest) ? dest
                                                 : periodic_transpose(dest));
    }
    ...
    void save_hdf5(H5::Group *root) const
    {
        ...
        throw NotSupported(
            "LatticeSpaceCellListImpl::save_hdf5 is not supported yet.");
    }
    ...
    void load_hdf5(const H5::Group &root)
    {
        ...
        throw NotSupported(
            "LatticeSpaceCellListImpl::load_hdf5 is not supported yet.");
    }
#endif
