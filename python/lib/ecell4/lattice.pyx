...
cdef class ReactionInfo:
    ...
    def __cinit__(self, Real t, reactants, products):
        ...
        for pid, p in reactants:
            reactants_.push_back(pair[Cpp_ParticleID, Cpp_Voxel](deref((<ParticleID>pid).thisptr), deref((<Voxel>p).thisptr)))
        for pid, p in products:
            products_.push_back(pair[Cpp_ParticleID, Cpp_Voxel](deref((<ParticleID>pid).thisptr), deref((<Voxel>p).thisptr)))
