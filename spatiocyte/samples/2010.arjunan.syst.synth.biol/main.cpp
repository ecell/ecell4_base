...
int main(int argc, char** argv)
{
    ...
    world->add_molecules((*model).create_species("MinD(p=adp,bs,loc=cyt)"), 1300);
    world->add_molecules((*model).create_species("MinD(p=atp,bs[1],loc=mem).MinEE(bs1[1],bs2,loc=mem)"), 700);
    SpatiocyteSimulator sim(model, world);

    SpatiocyteVisualizationLogger logger(world);

    logger.add_species((*model).create_species("MinEE(bs1,bs2,loc=mem)"));
    logger.add_species((*model).create_species("MinD(p=atp,bs[1],loc=mem).MinEE(bs1[1],bs2,loc=mem)"));
    logger.add_species((*model).create_species("MinD(p=atp,bs[1],loc=mem).MinEE(bs1[1],bs2[2],loc=mem).MinD(p=atp,bs[2],loc=mem)"));
    logger.add_species((*model).create_species("MinD(p=atp,bs,loc=mem)"));
    ...
    for (unsigned int i(0); i < 100; ++i)
    {
        ...
        std::cout << sim.t()
                  << "\t" << world->num_molecules((*model).create_species("MinEE(bs1,bs2,loc=mem)"))
                  << "\t" << world->num_molecules((*model).create_species("MinD(p=atp,bs[1],loc=mem).MinEE(bs1[1],bs2,loc=mem)"))
                  << "\t" << world->num_molecules((*model).create_species("MinD(p=atp,bs[1],loc=mem).MinEE(bs1[1],bs2[2],loc=mem).MinD(p=atp,bs[2],loc=mem)"))
                  << std::endl;
    }
