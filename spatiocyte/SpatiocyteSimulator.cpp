...
void SpatiocyteSimulator::save_hdf5(void)
{
	...
	{
		int tmp = 0;
		for (unsigned int j(0); j < this->world_->list_particles(species[i].first).size(); ++j)
		{
			h5_lattice_p[tmp + j].h5_lattice_id = static_cast<int>(this->world_->list_particles(species[i].first)[j].first.serial());

			std::strcpy(h5_lattice_p[tmp + i].h5_species_id, species[i].first.c_str());

//	      	h5_spatiocyte_p[tmp + j].h5_particle_position[0] = this->world_->list_particles(species[i].first)[j].second.position().elems[0];
//		    h5_spatiocyte_p[tmp + j].h5_particle_position[1] = this->world_->list_particles(species[i].first)[j].second.position().elems[1];
//		    h5_spatiocyte_p[tmp + j].h5_particle_position[2] = this->world_->list_particles(species[i].first)[j].second.position().elems[2];
...
//	mtype.insertMember(SPATIOCYTE_MEMBER2, HOFFSET(h5_particles, h5_particle_position), ArrayType(PredType::NATIVE_DOUBLE, 1, dims));
	...
	boost::scoped_ptr<Group> parent_group(new Group (this->file_->openGroup("/SpatiocyteWorld")));
	...
	H5::DataSet *dataset = new DataSet(this->file_->createDataSet(ost_hdf5path.str() + "/Particles", mtype, space));
