...
RUN cd /ecell4; git checkout -t origin/rulebase; PREFIX=/usr/local PYTHONPATH=/usr/local/lib/python2.7/dist-packages bash /ecell4/install.sh core core_python gillespie gillespie_python ode ode_python lattice lattice_python util_python bd bd_python
EXPOSE 8888
